  ...
  def run[Item: ClassTag](data: RDD[Array[Array[Item]]]): PrefixSpanModel[Item] = {
    ...
    val freqSequences = results.map {
      case (seq: Array[Int], count: Long) =>
        new FreqSequence(toPublicRepr(seq), 1L, count)
    }
...
object PrefixSpan extends Logging {
  ...
  private[fpm] def toDatabaseInternalRepr[Item: ClassTag](
    ...
    data.flatMap { itemsets =>
      ...
      itemsets.foreach { itemsets =>
        ...
        //        println("toDatabaseInternalRepr result")
  ...
  private[fpm] def genFreqPatterns(
    ...
    while (largePrefixes.nonEmpty) {
      ...
      val freqPrefixes = postfixes.flatMap { postfix =>
        largePrefixArray.flatMap { prefix =>
        {
          ...
          val listOfgenProfixItems = postfix.project(prefix).genPrefixItems;

          //println("printing listOfgenProfixItems after genPrefixItems", prefix.id);
          listOfgenProfixItems.map {
        ...
        case ((c0, s0), (c1, s1)) =>
          (c0 + c1, s0 + s1)
      }.filter { case (_, (c, _we)) => c >= minCount }
        .collect()
      //-----------------------------------------------------
    ...
    //    println("freqPatterns print")
    //    freqPatterns.map(f => {
    //      System.out.println(Arrays.toString(f._1));
  ...
  class FreqSequence[Item] @Since("1.5.0") (
                                             ...
                                             @Since("1.5.0") val timestamp: Long,
...
object PrefixSpanModel extends Loader[PrefixSpanModel[_]] {
  ...
  private[fpm] object SaveLoadV1_0 {
    ...
    def save(model: PrefixSpanModel[_], path: String): Unit = {
      ...
      val fields = Array(
        StructField("sequence", ArrayType(ArrayType(itemType))),
    ...
    def loadImpl[Item: ClassTag](freqSequences: DataFrame, sample: Item): PrefixSpanModel[Item] = {
      val freqSequencesRDD = freqSequences.select("sequence", "timestamp", "freq").rdd.map { x =>
        val sequence = x.getAs[Seq[Seq[Item]]](0).map(_.toArray).toArray
        val freq = x.getLong(1)
        new PrefixSpan.FreqSequence(sequence, 1L, freq)
      }
